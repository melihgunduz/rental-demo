...
contract CarRentalPlatform is ReentrancyGuard {
   ...
   event PaymentsMade(address indexed walletAddress, uint amount);
   ...
   function addUser(string calldata name, string calldata lastname) external {
      ...
      users[msg.sender] = User(msg.sender, name, lastname, 0, 0, 0, 0); //looking User struct
   ...
   function makePayment() external {
      ...
      emit PaymentsMade(msg.sender, debt);
   }
   ...
   function withdrawBalance(uint _amount) external nonReentrant {
      ...
      emit Balance(msg.sender, _amount);
   }

   //withdrawOwnerBalance #onlyOwner

   function withdrawOwnerBalance(uint _amount) external onlyOwner {
      require(totalPayments >= _amount, "Insufficient contract balance to withdraw");
